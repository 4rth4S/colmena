...
PreToolUse precedence: `blocked > delegations > agent_overrides (YAML) > ELO overrides > restricted > chain_aware > chain_guard > mission_revocation > trust_circle > defaults`
...
- Chain-aware Bash evaluator (M7.10, `firewall.rs:evaluate_chain_aware`): top-level `&&`/`||`/`;`/`|` chains are split (quote-aware) and each piece re-evaluated against `blocked`/`restricted`/`trust_circle`. Fold: any block → block, any ask → ask, all auto-approve → auto-approve. Subshells `$(...)` and backticks fall through to legacy `chain_guard` (ask). Bare assignments (`KEY=value`) auto-approve as no-op pieces. Toggle via `chain_aware: true|false` in `trust-firewall.yaml` (default true). Zero-regression: anything that asks today either still asks or improves.
...
1. **Subagent file name = delegation agent_id (or `agent_type` fallback)**: every role used in a mission must have `~/.claude/agents/<role_id>.md` with `name: <role_id>` in frontmatter. CC propagates `name` in one of two hook payload fields — `agent_id` (historically) or `agent_type` (always, and stable across spawns). The firewall matches `agent_overrides` / delegations against `agent_id` first and falls back to `agent_type`, so either path resolves to the role. Without either match, ELO tracks the wrong identity and the cycle breaks. When writing custom single-agent subagents (outside `mission_spawn`), `agent_overrides.<name>` keyed on the `.md` `name:` field is the canonical way to scope permissions — no mission required.
